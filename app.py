...
# MANUTENÇÃO: Deixamos a variável aqui para não quebrar referências, mas ela permanecerá vazia.
relatorio_simulacao = [] 
...
if temperaturas and duracao_simulacao > 0 and total_moradores_predio > 0:
    if st.sidebar.button("Executar Simulação"):
        ...
        for temperatura_atual in temperaturas:
            ...
            # Limpa o relatório a cada nova temperatura (manter para evitar erro de referência no loop)
            ...
            for i in range(n_simulacoes_maximo):
                # Inicializa o relatório para esta iteração. 
                # O relatório é preenchido AQUI, mas só será salvo se a condição (REMOVIDA) for satisfeita.
                ...
                # CORREÇÃO DE PERFORMANCE: Não salva mais o relatório, apenas verifica o BREAK
                if (convergencia_atingida or (i + 1) == n_simulacoes_maximo):
                    
                    # >>>>>>>>> BLOCO DE SALVAMENTO DE RELATÓRIO REMOVIDO AQUI <<<<<<<<<<
        ...
        # >>>>>>>>> BLOCO DE EXIBIÇÃO DE RELATÓRIO REMOVIDO AQUI <<<<<<<<<<
